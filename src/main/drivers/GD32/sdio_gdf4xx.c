...
static void SD_DataTransferInit(uint32_t Size, uint32_t DataBlockSize, bool IsItReadFromCard)
 {
    ...
    SDIO_DATATO = SD_DATATIMEOUT;       // Set the SDIO Data TimeOut value
    SDIO_DATALEN  = Size;                  // Set the SDIO DataLength value
...
static SD_Error_t SD_InitializeCard(void)
{
    ...
    if((SDIO_PWRCTL & SDIO_PWRCTL_PWRCTL) != 0) // Power off
    {
...
/**
  ...
  * @param  pDMA:         DMA channel to use for the DMA operation
...
/**
  ...
  */
// LOCAL_UNUSED_FUNCTION void SDIO_IRQHandler(void)
...
void SDIO_DMA_ST3_IRQHandler(dmaChannelDescriptor_t *dma)
{
    ...
    if((DMA_INTF0(DMA1) & DMA_INTF0_HTIF3) != 0)
    {
        ...
        {
            if(((DMA_CHCTL(DMA1, DMA_CH3)) & (uint32_t)(DMA_CHXCTL_SBMEN)) != 0)    // Multi_Buffering mode enabled
            {
                DMA_INTC0(DMA1) = DMA_INTC0_CHTIF3;                         // Clear the half transfer complete flag
            }
            else
            {
                if((DMA_CHCTL(DMA1, DMA_CH3) & DMA_CHXCTL_CMEN) == 0)             // Disable the half transfer interrupt if the DMA mode is not CIRCULAR
                {
                    DMA_CHCTL(DMA1, DMA_CH3)   &= ~DMA_CHXCTL_HTFIE;               // Disable the half transfer interrupt
                }
    ...
    if((DMA_INTF0(DMA1) & DMA_INTF0_TCIF3) != 0)
    {
        if((DMA_CHCTL(DMA1, DMA_CH3) & DMA_CHXCTL_FTFIE) != 0)
        {
            ...
            {
                DMA_INTC0(DMA1) = DMA_INTC0_CTCIF3;                         // Clear the transfer complete flag
            }
            else //Disable the transfer complete interrupt if the DMA mode is not CIRCULAR
            {
                if((DMA_CHCTL(DMA1, DMA_CH3) & DMA_CHXCTL_CMEN) == 0)
                {
                    DMA_CHCTL(DMA1, DMA_CH3) &= ~DMA_CHXCTL_FTFIE;                 // Disable the transfer complete interrupt
                }
...
void SDIO_DMA_ST6_IRQHandler(dmaChannelDescriptor_t *dma)
{
    ...
    if((DMA_INTF1(DMA1)  & DMA_HISR_TEIF6) != 0)
    {
        ...
        {
            DMA_CHCTL(DMA1, DMA_CH6)  &= ~DMA_CHXCTL_TAEIE;       // Disable the transfer error interrupt
            DMA_INTC1(dma_periph_sdio) = DMA_HIFCR_CTEIF6;             // Clear the transfer error flag
        }
    }
    ...
    if((DMA_INTF1(DMA1)  & DMA_HISR_FEIF6) != 0)
    {
        ...
        {
            DMA_CHFCTL(DMA1, DMA_CH6)   &= ~DMA_CHXFCTL_FEEIE;     // Disable the FIFO Error interrupt
            DMA_INTC1(dma_periph_sdio) = DMA_HIFCR_CFEIF6;             // Clear the FIFO error flag
        }
    }
    ...
    if((DMA_INTF1(DMA1)  & DMA_HISR_DMEIF6) != 0)
    {
        ...
        {
            DMA_CHCTL(DMA1, DMA_CH6)  &= ~DMA_CHXCTL_SDEIE;       // Disable the direct mode Error interrupt
            DMA_INTC1(dma_periph_sdio) = DMA_HIFCR_CDMEIF6;             // Clear the FIFO error flag
        }
    }
    ...
    if((DMA_INTF1(DMA1)  & DMA_HISR_HTIF6) != 0)
    {
        ...
        {
            if(((DMA_CHCTL(DMA1, DMA_CH6)) & (uint32_t)(DMA_CHXCTL_SBMEN)) != 0)    // Multi_Buffering mode enabled
            {
                DMA_INTC1(dma_periph_sdio) = DMA_HIFCR_CHTIF6;                         // Clear the half transfer complete flag
            }
            else
            {
                if((DMA_CHCTL(DMA1, DMA_CH6)& DMA_CHXCTL_CMEN) == 0)             // Disable the half transfer interrupt if the DMA mode is not CIRCULAR
                {
                    DMA_CHCTL(DMA1, DMA_CH6)&= ~DMA_CHXCTL_HTFIE;                 // Disable the half transfer interrupt
                }
    ...
    if((DMA_INTF1(DMA1)  & DMA_HISR_TCIF6) != 0)
    {
        if((DMA_CHCTL(DMA1, DMA_CH6)& DMA_CHXCTL_FTFIE) != 0)
        {
            ...
            {
                DMA_INTC1(dma_periph_sdio) = DMA_HIFCR_CTCIF6;                         // Clear the transfer complete flag
            }
            else //Disable the transfer complete interrupt if the DMA mode is not CIRCULAR
            {
                if((DMA_CHCTL(DMA1, DMA_CH6)& DMA_CHXCTL_CMEN) == 0)
                {
                    DMA_CHCTL(DMA1, DMA_CH6)  &= ~DMA_CHXCTL_FTFIE;               // Disable the transfer complete interrupt
                }
