...
static uint32_t spiDivisorToBRbits(const SPI_TypeDef *instance, uint16_t divisor)
{
    ...
    if (PERIPH_INT(instance) == PERIPH_INT(SPI1) || PERIPH_INT(instance) == PERIPH_INT(SPI2)) {
...
void spiInternalResetDescriptors(busDevice_t *bus)
{
    ...
    dma_single_data_para_struct_init(&dmaInitTx->config.init_struct_s);
    // dmaInitTx->DMA_Channel = bus->dmaTx->channel;
    dmaInitTx->config.init_struct_s.direction = DMA_MEMORY_TO_PERIPH;
    dmaInitTx->config.init_struct_s.circular_mode = DMA_CIRCULAR_MODE_DISABLE;
    dmaInitTx->config.init_struct_s.periph_addr = (uint32_t)&SPI_DATA((uint32_t)(bus->busType_u.spi.instance));
    dmaInitTx->config.init_struct_s.priority = DMA_PRIORITY_LOW;
    dmaInitTx->config.init_struct_s.periph_inc = DMA_PERIPH_INCREASE_DISABLE;
    dmaInitTx->config.init_struct_s.periph_memory_width = DMA_PERIPH_WIDTH_8BIT;

    dmaInitTx->data_mode = DMA_DATA_MODE_SINGLE;

    if (bus->dmaRx) {
        DMA_InitTypeDef *dmaInitRx = bus->initRx;

        dma_single_data_para_struct_init(&dmaInitRx->config.init_struct_s);
    //     dmaInitRx->DMA_Channel = bus->dmaRx->channel;
        dmaInitRx->config.init_struct_s.direction = DMA_PERIPH_TO_MEMORY;
        dmaInitRx->config.init_struct_s.circular_mode = DMA_CIRCULAR_MODE_DISABLE;
        dmaInitRx->config.init_struct_s.periph_addr = (uint32_t)&SPI_DATA((uint32_t)(bus->busType_u.spi.instance));
        dmaInitRx->config.init_struct_s.priority = DMA_PRIORITY_LOW;
        dmaInitRx->config.init_struct_s.periph_inc = DMA_PERIPH_INCREASE_DISABLE;
        dmaInitRx->config.init_struct_s.periph_memory_width = DMA_PERIPH_WIDTH_8BIT;

        dmaInitRx->data_mode = DMA_DATA_MODE_SINGLE;
    }
}
...
void spiInternalInitStream(const extDevice_t *dev, bool preInit)
{
    ...
    if (txData) {
        dmaInitTx->config.init_struct_s.memory0_addr = (uint32_t)txData;
        dmaInitTx->config.init_struct_s.memory_inc = DMA_MEMORY_INCREASE_ENABLE;
    } else {
        dummyTxByte = 0xff;
        dmaInitTx->config.init_struct_s.memory0_addr = (uint32_t)&dummyTxByte;
        dmaInitTx->config.init_struct_s.memory_inc = DMA_MEMORY_INCREASE_DISABLE;
    }
    dmaInitTx->config.init_struct_s.number = len;
    ...
    if (dev->bus->dmaRx) {
        ...
        if (rxData) {
            dmaInitRx->config.init_struct_s.memory0_addr = (uint32_t)rxData;
            dmaInitRx->config.init_struct_s.memory_inc = DMA_MEMORY_INCREASE_ENABLE;
        } else {
            dmaInitRx->config.init_struct_s.memory0_addr = (uint32_t)&dummyRxByte;
            dmaInitRx->config.init_struct_s.memory_inc = DMA_MEMORY_INCREASE_DISABLE;
        }

        dmaInitRx->config.init_struct_s.periph_memory_width = DMA_PERIPH_WIDTH_8BIT;

        dmaInitRx->config.init_struct_s.number = len;
    }
}
