...
// H7 uses DMAMUX-based request routing; each channel can be assigned any request
...
// Resolve UART/USART naming differences
...
#define TC(chan) DEF_TIM_CHANNEL(CH_ ## chan)
#define REQMAP_TIM(tim, chan) { (TIM_TypeDef *)tim, TC(chan), DMA_REQUEST_ ## tim ## _ ## chan }
...
// Reusable DMA channel specs for H7 DMAMUX-based assignment
