...
void cameraControlInit(void)
{
    ...
    if (CAMERA_CONTROL_MODE_HARDWARE_PWM == cameraControlConfig()->mode) {
        ...
        pwmOutConfig(&cameraControlRuntime.channel, timerHardware, timerClock((void *)TIMER5), CAMERA_CONTROL_PWM_RESOLUTION, 0, cameraControlRuntime.inverted);
        ...
        *cameraControlRuntime.channel.ccr = cameraControlRuntime.period;
        ...
        nvic_irq_enable(TIMER5_DAC_IRQn, NVIC_PRIORITY_BASE(NVIC_PRIO_TIMER), NVIC_PRIORITY_SUB(NVIC_PRIO_TIMER));
    ...
    if (CAMERA_CONTROL_MODE_HARDWARE_PWM == cameraControlConfig()->mode) {
        ...
        *cameraControlRuntime.channel.ccr = lrintf(dutyCycle * cameraControlRuntime.period);
        ...
        if (0 == hiTime) {
            ...
            timer_autoreload_value_config(TIMER5, cameraControlRuntime.period);
            timer_counter_value_config(TIMER6, 0);
            timer_autoreload_value_config(TIMER6, cameraControlRuntime.period);
