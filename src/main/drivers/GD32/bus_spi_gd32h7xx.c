...
static uint32_t spiDivisorToBRbits(const SPI_TypeDef *instance, uint16_t divisor)
{
    ...
    if (PERIPH_INT(instance) == PERIPH_INT(SPI3)) {
...
#define BR_BITS ((BIT(28) | BIT(29) | BIT(30)))
    ...
    SPI_CFG0(spi_periph) = (tempRegister | (spiDivisorToBRbits(instance, divisor) & BR_BITS));
#undef BR_BITS
#else
...
{
    DMA_InitTypeDef *dmaGenerInitTx = bus->initTx;
    ...
    if (bus->dmaRx) {
        DMA_InitTypeDef *dmaGenerInitRx = bus->initRx;
...
{
    static volatile busSegment_t *segmentLocal;
    if (preInit) {
        segmentLocal = dev->bus->curSegment;
    }
    volatile busSegment_t *segment = segmentLocal;
...
void spiInternalStartDMA(const extDevice_t *dev)
{
    ...
    if (dmaRx) {
        ...
        xDMA_Init(streamRegsTx, (dma_general_config_struct *)dev->bus->initTx);
#if defined(GD32F4)
        dma_channel_subperipheral_select((uint32_t)(dmaTx->dma), dmaTx->stream, dmaTx->channel);
#endif

        dma_general_config_struct * dmaGenerInitRx = (dma_general_config_struct *)dev->bus->initRx;
