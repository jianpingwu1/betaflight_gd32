...
bool ws2811LedStripHardwareInit(ioTag_t ioTag)
{
    ...
    timer_channel_output_mode_config((uint32_t)timer, timerHardware->channel, TIMER_OC_MODE_PWM0); // timerOCInit中会传入TIMER_OC_MODE_PWM0，也会进行一次
    ...
    dma_init_struct.periph_memory_width = DMA_PERIPH_WIDTH_32BIT; // periph &　memory
