...
static void adcInitInternalInjected(const adcConfig_t *config)
{
    ...
    // adc_inserted_channel_config(adc_periph, 2, ADC_CHANNEL_20, 638); // ADC_Channel_HP_TempSensor
...
#else
    dma_init_struct.sub_periph = dmaSpec->channel;
#endif
#else
#if defined(GD32H7)
    dma_init_struct.config.init_struct_s.request = adc.channel;
#else
    dma_init_struct.sub_periph = adc.channel;
#endif
#endif
