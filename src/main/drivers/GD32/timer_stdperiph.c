...
#define CC_CHANNELS_PER_TIMER 4                 // TIMER_CH_0..3
...
int8_t timerGetTIMNumber(const void *tim)
{
...
void timerReconfigureTimeBase(void *tim, uint16_t period, uint32_t hz)
{
    configTimeBase(tim, period, hz);
}

void configTimeBase(void *tim, uint16_t period, uint32_t hz)
{
    ...
    // "The counter clock frequency (CK_CNT) is equal to f CK_PSC / (PSC[15:0] + 1)." - GD32F40x Reference Manual 14.4.11
    // Thus for 1Mhz: 200000000 / 1000000 = 200, 200 - 1 = 199 = TIM_Prescaler
...
void timerConfigUpdateCallback(const void *tim, timerOvrHandlerRec_t *updateCallback)
{
...
static void timCCxHandler(void *tim, timerConfig_t *timerConfig)
{
    uint16_t capture;
    unsigned tim_status;
    tim_status = TIMER_INTF(&tim) & TIMER_DMAINTEN(&tim);
    ...
    while (tim_status) {
        ...
        switch (bit) {
            ...
            case __builtin_clz(TIMER_INT_CH0):
                timerConfig->edgeCallback[0]->fn(timerConfig->edgeCallback[0], TIMER_CH0CV(&tim));
                break;
            case __builtin_clz(TIMER_INT_CH1):
                timerConfig->edgeCallback[1]->fn(timerConfig->edgeCallback[1], TIMER_CH1CV(&tim));
                break;
            case __builtin_clz(TIMER_INT_CH2):
                timerConfig->edgeCallback[2]->fn(timerConfig->edgeCallback[2], TIMER_CH2CV(&tim));
                break;
            case __builtin_clz(TIMER_INT_CH3):
                timerConfig->edgeCallback[3]->fn(timerConfig->edgeCallback[3], TIMER_CH3CV(&tim));
                break;
        }
    }
#else
    if (tim_status & (int)TIMER_INT_UP) {
        TIMER_INTF(&tim) = ~TIMER_INT_UP;
        capture = TIMER_CAR(&tim);
        timerOvrHandlerRec_t *cb = timerConfig->overflowCallbackActive;
        while (cb) {
            cb->fn(cb, capture);
    ...
    if (tim_status & (int)TIMER_INT_CH0) {
        TIMER_INTF(&tim) = ~TIMER_INT_CH0;
        timerConfig->edgeCallback[0]->fn(timerConfig->edgeCallback[0], TIMER_CH0CV(&tim));
    }
    if (tim_status & (int)TIMER_INT_CH1) {
        TIMER_INTF(&tim) = ~TIMER_INT_CH1;
        timerConfig->edgeCallback[1]->fn(timerConfig->edgeCallback[1], TIMER_CH1CV(&tim));
    }
    if (tim_status & (int)TIMER_INT_CH2) {
        TIMER_INTF(&tim) = ~TIMER_INT_CH2;
        timerConfig->edgeCallback[2]->fn(timerConfig->edgeCallback[2], TIMER_CH2CV(&tim));
    }
    if (tim_status & (int)TIMER_INT_CH3) {
        TIMER_INTF(&tim) = ~TIMER_INT_CH3;
        timerConfig->edgeCallback[3]->fn(timerConfig->edgeCallback[3], TIMER_CH3CV(&tim));
    }
#endif
}
...
_TIM_IRQ_HANDLER_UPDATE_ONLY(TIMER5_IRQHandler, 5);
#  endif
#endif
...
// _TIM_IRQ_HANDLER(TIMER7_UP_IRQHandler, 7);
...
#endif
#if USED_TIMERS & TIM_N(14)
_TIM_IRQ_HANDLER(TIMER0_BRK_TIMER14_IRQHandler, 14);
#endif
#if USED_TIMERS & TIM_N(16)
_TIM_IRQ_HANDLER(TIMER0_TRG_CMT_TIMER16_IRQHandler, 16);
#endif
...
void timerForceOverflow(TIM_TypeDef *tim)
{
    ...
    ATOMIC_BLOCK(NVIC_PRIO_TIMER) {
        ...
        timerConfig[timerIndex].forcedOverflowTimerValue = TIMER_CNT(&tim) + 1;
...
void timerOCPreloadConfig(void *tim, uint8_t channel, uint16_t ocshadow)
{
    switch (channel) {
    case TIMER_CH_0:
        timer_channel_output_shadow_config((uint32_t)tim, TIMER_CH_0, ocshadow);
        break;
    case TIMER_CH_1:
        timer_channel_output_shadow_config((uint32_t)tim, TIMER_CH_1, ocshadow);
        break;
    case TIMER_CH_2:
        timer_channel_output_shadow_config((uint32_t)tim, TIMER_CH_2, ocshadow);
        break;
    case TIMER_CH_3:
        timer_channel_output_shadow_config((uint32_t)tim, TIMER_CH_3, ocshadow);
        break;
    }
}
#endif
