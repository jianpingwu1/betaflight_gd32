...
void uartReconfigure(uartPort_t *uartPort)
{
    ...
    if (uartPort->port.mode & MODE_RX) {
        ...
        if (uartPort->rxDMAResource) {
            ...
            dma_init_struct.config.init_struct_s.periph_addr = uartPort->rxDMAPeripheralBaseAddr;
            dma_init_struct.config.init_struct_s.memory0_addr = (uint32_t)uartPort->port.rxBuffer;
            dma_init_struct.config.init_struct_s.number = uartPort->port.rxBufferSize;
            ...
            dma_init_struct.config.init_struct_s.periph_memory_width = DMA_PERIPH_WIDTH_8BIT;
            ...
            dma_init_struct.config.init_struct_s.priority = DMA_PRIORITY_MEDIUM;

            dma_init_struct.data_mode = DMA_DATA_MODE_SINGLE;

#if defined(GD32H7)
            dma_init_struct.config.init_struct_s.request = uartPort->rxDMAChannel;
#else
            uint32_t temp_dma_periph;
            int temp_dma_channel;
            gd32_dma_chbase_parse((uint32_t)uartPort->rxDMAResource, &temp_dma_periph, &temp_dma_channel);

            dma_channel_subperipheral_select(temp_dma_periph, temp_dma_channel, uartPort->rxDMAChannel);
            dma_init_struct.sub_periph = uartPort->rxDMAChannel;
#endif

            xDMA_DeInit(uartPort->rxDMAResource);
            // gd32_dma_init((uint32_t)uartPort->rxDMAResource, &dma_init_struct);
            xDMA_Init((uint32_t)uartPort->rxDMAResource, &dma_init_struct);
    ...
    if (uartPort->port.mode & MODE_TX) {
        ...
        if (uartPort->txDMAResource) {
            ...
            dma_init_struct.config.init_struct_s.periph_addr = uartPort->txDMAPeripheralBaseAddr;
            dma_init_struct.config.init_struct_s.memory0_addr = (uint32_t)uartPort->port.txBuffer;
            dma_init_struct.config.init_struct_s.number = uartPort->port.txBufferSize;
            dma_init_struct.config.init_struct_s.periph_inc = DMA_PERIPH_INCREASE_DISABLE;
            dma_init_struct.config.init_struct_s.memory_inc = DMA_MEMORY_INCREASE_ENABLE;
            dma_init_struct.config.init_struct_s.periph_memory_width = DMA_PERIPH_WIDTH_8BIT;
        ...
        uint32_t temp_dma_periph;
        int temp_dma_channel;
        gd32_dma_chbase_parse((uint32_t)uartPort->txDMAResource, &temp_dma_periph, &temp_dma_channel);

        dma_channel_subperipheral_select(temp_dma_periph, temp_dma_channel, uartPort->txDMAChannel);
        dma_init_struct.sub_periph = uartPort->txDMAChannel;
#endif

            xDMA_DeInit(uartPort->txDMAResource);
            xDMA_Init((uint32_t)uartPort->txDMAResource, &dma_init_struct);
