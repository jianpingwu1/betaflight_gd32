...
void uartReconfigure(uartPort_t *uartPort)
{
    ...
    if (uartPort->port.mode & MODE_RX) {
        ...
        if (uartPort->rxDMAResource) {
            ...
            xDMA_Init(uartPort->rxDMAResource, &dma_init_struct);
    ...
    if (uartPort->port.mode & MODE_TX) {
        ...
        if (uartPort->txDMAResource) {
            ...
            xDMA_Init(uartPort->txDMAResource, &dma_init_struct);
...
void uartTryStartTxDMA(uartPort_t *s)
{
    ATOMIC_BLOCK(NVIC_PRIO_SERIALUART_TXDMA) {
        if (IS_DMA_ENABLED(s->txDMAResource)) {
            return; // DMA is already in progress
        }

        if (xDMA_GetCurrDataCounter(s->txDMAResource)) {
            goto reenable; // Possible premature TC case.
        }
