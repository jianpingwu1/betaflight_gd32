...
void mcoConfigure(MCODevice_e device, const mcoConfig_t *config)
{
...
#if defined(STM32F4) || defined(STM32F7) || defined(GD32F4) || defined(GD32H7)
    ...
    IOConfigGPIOAF(io, IO_CONFIG(GPIO_MODE_AF, GPIO_OSPEED_60MHZ, GPIO_OTYPE_PP, GPIO_PUPD_NONE), GPIO_AF_0);
#else
        // All F4s
